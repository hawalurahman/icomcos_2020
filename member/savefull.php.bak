...
if ($_GET['act']=='save')
{
	$lokasi_file = $_FILES['fuploadabs']['tmp_name'];
	$nama_file   = $_FILES['fuploadabs']['name'];
	$acak = rand(0000000,9999999);
	$nama_file_acak = $acak.$nama_file;
	$type_file   = $_FILES['fuploadabs']['type'];
	$size_file   = $_FILES['fuploadabs']['size'];
	if (!empty($lokasi_file)){
		move_uploaded_file($lokasi_file,"../images/$nama_file_acak");
		mysql_query("UPDATE peserta SET fileabstrak='$nama_file_acak' WHERE iduser='$_SESSION[iduser]' and thnsnma=$_SESSION[thnsnma]");
		$utama ="<br><br><br><center>
				<font color=#F20D24 size=4><b>ABSTRAK BERHASIL DI UPLOAD..!</b></font><br><br>
				</center>";
